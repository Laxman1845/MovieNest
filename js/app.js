  ...
  var options = {
    ...
    handler: async function (response) {
      ...
      try {
        ...
        console.error(err);
        showCustomPopup(
          err.message || "Booking conflict detected. Please try again.",
          "error",
        );
        router("seats", { movie, timeSlot });
      }
  ...
  querySnapshot.forEach((docSnap) => {
    ...
    card.innerHTML = `
      <div>
        ...
        <h3 class="text-lg font-bold">${booking.movieTitle || "Movie"}</h3>
        <p class="text-sm text-slate-400">Time Slot: ${booking.timeSlot}</p>
...
window.showCustomPopup = function (message, type = "info") {
  ...
  popup.innerHTML = `
    <div class="fixed top-6 right-6 z-[9999] max-w-sm w-[calc(100%-2rem)]">
      <div class="bg-slate-900 border border-slate-700 rounded-xl shadow-2xl p-4 flex items-start gap-3">

        <div class="w-9 h-9 shrink-0 rounded-full ${iconClass} flex items-center justify-center">
          ${icon}
        </div>

        <p class="text-sm text-slate-200 leading-relaxed flex-1 pt-1">
          ${message}
        </p>
