...
/* ============================================================
   RAZORPAY PAYMENT (backend-verified order + signature)
   ============================================================ */
...
window.initiateRazorpayPayment = async function (movie, timeSlot) {
  ...
  /*
   * Before creating the order, verify that every
   * selected seat still belongs to this user.
   */
  ...
  /*
   * Ask the backend to create the Razorpay order.
   * The backend is the source of truth for pricing
   * and seat availability at order-creation time.
   */
...
/* ============================================================
   ...
   (verified server-side via /api/bookings)
   ============================================================ */

async function completeBookingAfterPayment(movie, timeSlot, seatIds, paymentRes) {
  if (!currentUser) {
  ...
  try {
    ...
    /*
     * The booking is now permanent server-side.
     * Release the temporary client-side seat locks.
     */
     ...
     * but backend confirmation may have failed.
     *
